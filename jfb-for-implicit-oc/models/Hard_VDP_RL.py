...
models.Hard_VDP_RL
-------------------
Hard Van der Pol oscillator stabilisation in the RL setting.

Same dynamics as VanDerPolOC_RL (ẋ₂ = (1 − x₁²)x₂ − x₁ + u), but with an
exponential control cost:
    L = x₁² + x₂² + lambda_u · (exp(u²) − 1)

∇_u H = 2 lambda_u u exp(u²) + p₂ is transcendental — no closed-form u*, so
the implicit fixed-point iteration is genuinely required.
"""
